...
void usage(int exit_code) {
    printf("USAGE: smac [" ARG_HOST_URL_LONG "] <host> [OPTS]\n"
           "OPTS:\n"
           "    TARGET:\n"
           "        " ARG_HOST_URL_LONG " <URL> : target host\n"
           "\n"
           "    SCAN MODE:\n"
           ...
           "        " ARG_MAC_FILE_LONG " <FILE> : scan MACs contained in FILE\n"
           "\n"
           "    REQUESTS:\n"
           "        " ARG_REQUEST_DELAY_LONG   " <MILLIS> : delay after each request\n"
           "        " ARG_PAUSE_COUNT_LONG     " <INT> : pause every INT request\n"
           "        " ARG_PAUSE_DELAY_LONG     " <MILLIS> : pause duration\n"
           "        " ARG_REQUEST_TIMEOUT_LONG " <MILLIS> : set timeout (default = 3000 ms)\n"
           "        " ARG_STOP_COUNT_LONG          " <INT> : stop after INT requests (default = 0)\n"
           "        " ARG_MAX_RETRY_LONG " <INT> : retry failed request INT times (default = -1)\n"
           "        " ARG_THREADS_LONG             " <INT> : parallel requests (default = 1)\n"
           "\n"
           "    PROXIES:\n"
           "        " ARG_MANUAL_PROXY_LONG " <PROXY> : use PROXY for all requests\n"
           "            |_ " ARG_PROXY_USERNAME_LONG " <USER> : optional\n"
           "            |_ " ARG_PROXY_PASSWORD_LONG " <PASS> : optional\n"
           "        " ARG_PROXY_FILE_LONG " <FILE> : use list of proxies from FILE (automatic rotation)\n"
           "        " ARG_PROXY_FROM_URL_LONG " <URL> : use list of proxies from URL (automatic rotation)\n"
           "\n"
           "    FILTER:\n"
           "        " ARG_GENRE_MATCH_LONG " <STR> : skip account when no genre matches STR\n"
           "        " ARG_PLAYABLE_LONG " : skip account if streams are not playable (ffprobe)\n"
           "\n"
           "    RESULTS:\n"
           "        " ARG_SAVE_DIR_LONG " <DIR> : set folder for results and checkpoints storage\n"
           "        " ARG_NO_CHECKPOINT_LONG " : do not use nor save checkpoints during this scan\n"
           "\n"
           "    MISC:\n"
           "        " ARG_MAC_PREFIX_LONG " <PREFIX> : generate MACs starting with PREFIX during [" ARG_RANDOM_SCAN_LONG "] scan\n"
           );
    exit(exit_code);
}
